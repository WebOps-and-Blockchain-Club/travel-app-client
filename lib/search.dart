...
// import 'flights.dart';
List<List> path = [
  ["place1", "place2", "air", 36005.0, 15000.0],
  ["place2", "place3", "air", 36005.0, 15000.0],
  ["place3", "place4", "rail", 36005.0, 15000.0]
];
            ...
            child: Column(
              ...
              children: <Widget>[
                TextField(
                  controller: start,
                  decoration:
                      const InputDecoration(labelText: 'Enter start location'),
                ),
                // ElevatedButton(
                //   onPressed: () {
                ...
                TextField(
                  controller: end,
                  decoration:
                      const InputDecoration(labelText: 'Enter end location'),
                ),
                Row(
                ...
                Center(
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      // });
                      await getTrains(
                          start_railway, end_railway, dateInput.text);
                      searchFlights(start_airport, end_airport, dateInput.text,
                          dateInput.text);
                    },
...
Future<void> getTrains(start, end, date) async {
  ...
  try {
    final response = await http.get(
      Uri.parse(
          'http://indian-railway-api.cyclic.app/trains/gettrainon?from=$start_railway&to=$end_railway&date=${dateInput.text}'),
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < data["data"].length; i++) {
        ...
        String city_departure = await getStationInfo(
            data["data"][i]["train_base"]["from_stn_code"]);
        String city_arrival =
            await getStationInfo(data["data"][i]["train_base"]["to_stn_code"]);

        double price = await getFare(
            data["data"][i]["train_base"]["from_stn_code"],
            data["data"][i]["train_base"]["to_stn_code"],
            data["data"][i]["train_base"]["train_no"]);
...
Future<void> searchFlights(dep, arr, out_date, return_date) async {
  String apiUrl = 'http://10.0.2.2:3000/search_flights';
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < data["best_flights"].length; i++) {
        ...
        double distance = await getAirportDistance(
            ...
            // )
            await getMAACoordinates(data["best_flights"][i]["flights"][0]
                ["departure_airport"]["id"]),
            await getMAACoordinates(data["best_flights"][i]["flights"][0]
                ["arrival_airport"]["id"]));

        print("hereeee2");
        String city_departure = await getAirportCity(
            data["best_flights"][i]["flights"][0]["departure_airport"]["id"]);
        String city_arrival = await getAirportCity(
            data["best_flights"][i]["flights"][0]["arrival_airport"]["id"]);
        ...
        graph.add([
          ...
          extractTime(data["best_flights"][i]["flights"][0]["departure_airport"]
              ["time"]),
          extractTime(
              data["best_flights"][i]["flights"][0]["arrival_airport"]["time"]),
          extractTime(
              data["best_flights"][i]["flights"][0]["arrival_airport"]["time"]),
          distance,
      ...
      for (int i = 0; i < data["other_flights"].length; i++) {
        ...
        double distance = await getAirportDistance(
            ...
            //     data["other_flights"][i]["flights"][0]["arrival_airport"]["id"]
            // )
            await getMAACoordinates(data["other_flights"][i]["flights"][0]
                ["departure_airport"]["id"]),
            await getMAACoordinates(data["other_flights"][i]["flights"][0]
                ["arrival_airport"]["id"]));
        ...
        graph.add([
          ...
          extractTime(data["other_flights"][i]["flights"][0]
              ["departure_airport"]["time"]),
          extractTime(data["other_flights"][i]["flights"][0]
              ["departure_airport"]["time"]),
          extractTime(data["other_flights"][i]["flights"][0]["arrival_airport"]
              ["time"]),
          distance,
...
Future<double> getDistance(start, end) async {
  final apiKey =
      "e03ce064fe991e8754bd996dbf7c8d5d3e0b8ab93cb1217e10d6a05549f428fe";
  ...
  try {
    final response = await http.get(Uri.parse(
        "https://serpapi.com/search.json?api_key=$apiKey&engine=$engine&hl=$hl&q=$q&gl=$gl&start_addr=$startAddr&end_addr=$endAddr&travel_mode=$travelMode&distance_unit=$distanceUnit"));
...
Future<String> getAirportCoordinates(airport) async {
  ...
  const String apiKey =
      '0509bbc2-21b1-4fd9-8d18-393015d21bf7'; // Replace with your AviationStack API key
...
Future<double> getAirportDistance(String start, String end) async {
  final response = await http.get(
    Uri.parse(
        'https://router.project-osrm.org/route/v1/driving/$start;$end?steps=true'),
    headers: {
...
Future<String> getStationCode(String station) async {
  final apiKey =
      "e03ce064fe991e8754bd996dbf7c8d5d3e0b8ab93cb1217e10d6a05549f428fe";
  final url = Uri.parse(
      "https://serpapi.com/search.json?engine=google&q=main%20railway%20station%20in%20${station}%20code&location=Austin,%20Texas,%20United%20States&google_domain=google.com&gl=us&hl=en&api_key=$apiKey");
...
Future<void> sendGraph(graph) async {
  String url = 'http://10.0.2.2:3000/getroute';

  Map<String, dynamic> requestBody = {
    'start': start.text,
    'destination': end.text,
    'currTime': DateTime.now().hour < 10
...
Future<String> getMAACoordinates(airport) async {
  ...
  String url =
      "https://serpapi.com/search.json?api_key=$api_key&engine=$engine&q=$q&location=$location&google_domain=$google_domain&gl=$gl&hl=$hl";
